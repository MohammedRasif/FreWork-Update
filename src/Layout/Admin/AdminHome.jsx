...
const AdminHome = () => {
  ...
  const handleSubmitOffer = async (planId, budget, comment) => {
    ...
    if (offeredBudget < planBudget - 500 || offeredBudget > planBudget + 500) {
      toast.error(
        `Offered budget must be within ±$500 of the plan budget ($${planBudget}). Your offer ($${offeredBudget}) is outside the range $${
          planBudget - 500
        } to $${planBudget + 500}.`,
        { duration: 5000 }
      );
